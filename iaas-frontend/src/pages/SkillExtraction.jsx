...
export default function SkillExtraction() {
  ...
  async function handleUploadJDFile() {
    if (!jdId) { setError('Invalid JD id.'); return }
    if (!selectedFile) { setError('Please choose a .pdf or .docx file.'); return }
    ...
    try {
      ...
      await uploadJDFile(jdId, selectedFile)
  ...
  async function handleDeleteCard(localId) {
    ...
    try {
      setError('')
      if (!card.isNew) await deleteSkill(jdId, card.id)
  ...
  function handleCancelEdit(localId) {
    setSkillCards((previous) => {
      ...
      if (card.isNew) return previous.filter((entry) => entry.local_id !== localId)

      return previous.map((entry) => {
        if (entry.local_id !== localId) return entry
        const snapshot = entry.editSnapshot
        if (!snapshot) return { ...entry, isEditing: false }
        return {
          ...entry,
  ...
  async function handleSaveCard(localId) {
    ...
    if (!card.skill_name.trim()) { setError('Skill name is required.'); return }
    ...
    try {
      ...
      if (card.isNew) {
        await addSkill(jdId, { ...payload, skill_type: card.skill_type || 'secondary' })
  ...
  function getExportFileBaseName() {
    return `jd-${jdId || 'unknown'}-skills-${new Date().toISOString().slice(0, 10)}`
  }

  function handleDownloadExcel() {
    const rows = getExportRows()
    if (rows.length === 0) { setError('No skills available to export.'); return }

    try {
      setIsDownloadingExcel(true)
      setError('')
      const worksheet = XLSX.utils.json_to_sheet(
        rows.map((row) => ({ Type: row.type, 'Skill Name': row.skill_name, Importance: row.importance, Subtopics: row.subtopics })),
      )
    ...
    <AppShell pageTitle="Skill Extraction" pageSubtitle={jd?.title || `JD #${jdId}`}>
      ...
      <Card>
        ...
        <div className="raw-text-viewer mb-4">
          {(jd?.raw_text && jd.raw_text.trim()) ? rawPreview : 'No raw text available.'}
        </div>
